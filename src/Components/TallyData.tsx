...
export const Statistics = () => {
    // covid Data is collected from rtk-query
    const { data, isLoading, isError } = useGetWorldWideDataQuery("")
    if (isLoading) return <h1>Loading...</h1>
    return <>
        {isError && <div>Something went wrong</div>}
        {
            data && <section className="bg-white dark:bg-gray-900   lg:ml-36  mb-5">
                <div className="max-w-screen-xl px-4 py-8 mx-auto text-center lg:py-16 lg:px-6">
                    <dl className="grid max-w-screen-md gap-20 md:ml-36 lg:mx-auto text-gray-900  lg:grid-cols-2   dark:text-white">
