...
import { useAddContactsMutation, useDeleteContactsMutation, useGetContactsQuery, useUpdateContactsMutation } from '../Api/ContactSlice'
...
const ContactTable = (prop: { data: [ContactType] }) => {
    ...
    return <>
        <section className="p-3 sm:p-5 antialiased  md:ml-32 sm:ml-20  ">
                ...
                <div className="bg-white dark:bg-gray-800 relative   shadow-md sm:rounded-lg overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                {
                                    prop.data && prop.data.map((item: ContactType) => <tr className="border-b dark:border-gray-700" key={item.id}>
                                        <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{item.firstname}</th>
                                        <td className="px-4 py-3 text-slate-100">{item.lastname}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            ...
                                            <button onClick={async e => await (deleteContacts(item.id as number))}
                                                type="submit" className="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center bg-red-600 text-white bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800">
                                                Delete
                                            </button>
                                        </td>
        ...
        <div id="defaultModal" tabIndex={-1} aria-hidden="true" className={
            ...
            <div className=" p-4 w-full max-w-2xl h-full md:h-auto ">
                ...
                <div className="relative p-4 bg-white rounded-lg shadow dark:bg-blue-800 sm:p-5">
                    ...
                    <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                        ...
                        <button onClick={e => setShowEditModal(false)} type="button" className="text-slate-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="defaultModal">
                            <svg aria-hidden="true" className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                            <span className="sr-only">Close modal</span>
                        </button>
                    </div>
                    ...
                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            <label htmlFor="name" className="block mb-2 text-sm font-medium text-slate-100 dark:text-white"> First Name</label>
                            <input type="text" value={selectedContact.firstname} name="name" id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-slate-300 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type First name" required onChange={e => setSelectedContact({ ...selectedContact, firstname: e.target.value })} />
                        </div>
                        <div>
                            <label htmlFor="brand" className="block mb-2 text-sm font-medium text-slate-50 dark:text-white">Last Name</label>
                            <input value={selectedContact.lastname} type="text" name="brand" id="brand" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-slate-300 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type Last Name" required onChange={e => setSelectedContact({
                                ...selectedContact,
                                lastname: e.target.value
                            })} />
                        </div>
                        <div>
                            <label htmlFor="brand" className="block mb-2 text-sm font-medium text-slate-50 dark:text-white">Status</label>
                            <div className="flex items-center mb-4">
                                <input
                            ...
                            <div className="flex items-center">
                                <input
                                    checked={!selectedContact.status}
                                    id="default-radio-2" type="radio" value="inactive" name="status" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-slate-100 dark:border-gray-600" onChange={e => setSelectedContact({
                                        ...selectedContact,
                                        status: e.target.value !== "inactive"
                                    })} />
                                <label htmlFor="default-radio-2" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Inactive</label>
                            ...
                            <div className="flex justify-center">
                                <button onClick={handleUpdateContact} type="submit" className="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center bg-slate-100 text-black bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800">
                                    Update Contact
                                </button>
                            </div>
                        </div>
